...
+-----------------+
      ...
      v N
+-----------------+     +-----------------+     +-----------------+
|     EVENT       |     |    CATEGORY     |     |   ENROLMENT     |
+-----------------+     +-----------------+     +-----------------+
| event_id PK     |-----<| category_id PK  |-----<| user_id FK      |
| event_name      |     | event_id FK     |     | event_id FK     |
| event_date      |     | category_name   |     | category_id FK  |
| event_time      |     | min_age         |     | enrolment_date  |
| venue           |     | max_age         |     | enrolment_status|
| location        |     | gender_restr    |     | payment_status  |
| distance_km     |     | max_participants|     | amount_paid     |
| event_type      |     | entry_fee       |     | payment_date    |
| entry_fee       |     | created_by FK   |     | created_by FK   |
| status          |     | created_at      |     +-----------------+
| created_by FK   |     +-----------------+
| created_at      |           |
| updated_at      |           | 1
+-----------------+           v N
      ...
      | 1             |        PARTICIPANT_EVENT                |
      |               +-----------------------------------------+
      v N             | participant_event_id PK                 |
+-----------------+   | event_id FK                             |
|     WEATHER     |   | user_id FK                              |
+-----------------+   | category_id FK                          |
| weather_id PK   |   | bib_number                              |
| event_id FK     |   | gate_position                           |
| forecast_date   |   | finishing_position                      |
| temperature     |   | finish_time_seconds                     |
| humidity        |   | pace_per_km                             |
| wind_speed      |   | entry_status                            |
| wind_direction  |   | entered_at                              |
| precip_chance   |   | updated_by FK                           |
| conditions      |   +-----------------------------------------+
| forecast_time   |                    |
+-----------------+                    | 1
      |                                 v N
...
+-----------------+     +-----------------+
| route_id PK     |-----<| waypoint_id PK  |
| event_id FK     |     | route_id FK     |
| route_name      |     | latitude        |
| distance_km     |     | longitude       |
| terrain_type    |     | sequence_order  |
| elevation_gain  |     | waypoint_type   |
| elevation_loss  |     | description     |
| route_desc      |     +-----------------+
| gpx_data        |
| start_lat       |
| start_long      |
